...
int main(int argc, char *argv[]) {
	...
	int opt_char;
	int option_index;
	struct option long_options[4] = {{"show-pids", no_argument, 0, 'p'},
									 {"numeric-sort", no_argument, 0, 'n'},
									 {"version", no_argument, 0, 'V'}};
	while ((opt_char = getopt_long(argc, argv, "pnV", long_options,
								   &option_index)) != -1) {
		switch (opt_char) {
		case 'p':
			pid = 1;
...
void build_node_(struct ProcessTree *node) {
	...
	for (i = 0; i < PROC_MAX && j < PROC_SON; ++i) {
		...
		if (procs[i].ppid == node->proc->pid) {
			node->sons[j] =
				(struct ProcessTree *)malloc(sizeof(struct ProcessTree));
			int k;
			for (k = 0; k < PROC_SON; ++k)
...
void build() { build_node_(process_tree); }
...
void print_layer_(struct ProcessTree *node, int start, int sib, int pid,
				  int space, char *line) {
	line[space] = line[space + 1] = line[space + 2] = line[space + 3] =
		line[space + 4] = ' ';
	if (start) {
		if (sib) {
			printf("─┬─");
